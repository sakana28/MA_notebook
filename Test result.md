...
This section evaluates the system resource utilization based on the Vivado report. As shown in figure \ref{fig:resource summary}, the overall FPGA resource usage of the system is very low. 所有类型的资源被使用的数量都在2.5%以下。其中被使用最多的是IO。而section x提到了，当前的设计中有4个IO只用于debug。因此，IO的使用量可以被降到4个，即使用了全部可用IO的1.1%。This allows for future system expansion or direct integration into a vibration signal processing and analyzing system. It also allows multiple instances of the same design on a single development board to serve as signal sources for multiple downstream systems in some application scenarios.

The resource usage report for each component in figure \ref{fig:resource} shows that the custom IP module consumes the highest number of LUTs and registers，分别占全部使用量的29.7%和20.2%。 Within the design, there are potential optimizations for the Custom IP resource usage. In the KX134 sensor, several registers are reserved and serve no practical function, moreover, and a large number of registers remain unused in the current design. In future optimizations of this system, resource efficiency could be improved by reducing the number of registers to the actual need in the custom IP, combined with an extra address decoding module.