...
## Conclusion
本论文介绍了使用 Xilinx Zynq SoC 平台设计和实施滚动轴承振动信号发生器和记录器系统的情况。该系统的目的是为振动信号的处理和故障分析提供可定制的与硬件集成的数据，对于未来的预测性维护解决方案的开发有重要的作用。该系统的论文首先分析了对滚动轴承振动信号建模的背景以及数值实现，并分析了该建模在

该系统的核心包括用于模拟信号生成的 Python 脚本、用于配置和控制的 C 软件以及实现硬件数据路径和逻辑的 VHDL 模块。信号发生器模拟加速度计，输出从文本文件中获取的人工振动数据。信号记录器通过集成的加速度计获取真实的振动测量值。两个子系统使用相同的软件和通信接口，以确保一致性。


这项工作的一个主要贡献是用 VHDL 开发了一个定制的 I2C 从属外设。它在信号发生器数据路径中模拟了加速度计功能。定制的从 IP 能够配置采样参数，并使用 AXI I2C 控制器通过 I2C 读取数据。其有限状态机精确地遵循了 I2C 协议规范，以实现稳健的事务处理。通过时钟分频和缓冲逻辑，定制内核生成了灵活的 I2C 传感器接口。


软件组件利用赛灵思库，包括独立操作系统、scugic 中断控制器、用于高速数据传输的 axi_dma，以及用于 SD 卡访问的 FatFs 文件系统。精心的配置实现了高效的硬件控制和数据移动。分层架构将硬件交互、文件 I/O 和应用流等关注点分开。Python 脚本可快速生成模拟信号文本数据。


在工具方面，Vivado 促进了 IP 集成和逻辑综合，以便对 PL 结构进行编程。Vitis IDE 可以设计、编译和调试在 ARM 处理器上执行的 C 软件。两者的结合实现了从模型到比特流的嵌入式系统开发。


综合实验成功验证了信号路径功能。来自 SD 卡的文本数据与记录的输出相匹配。这证明了子系统的连接性，并验证了 I2C、AXI、中断和 SD 操作的正确性。不过，对信号保真度的详细分析仍是今后的工作重点。


虽然概念上简单明了，但在硬件中实现不同协议交互时还是出现了微妙的复杂性。例如，管理 AXI-Stream 和 I2C 总线的握手信号和时钟域交叉需要仔细考虑时序问题。要找出集成问题的根本原因，必须进行细致的调试。


该项目是一次宝贵的嵌入式系统设计学习经历。它提供了将 FPGA 软处理器与 SoC 相结合的许多软硬件接口方面的经验。这个多组件平台的复杂性凸显了在信号级理解 AXI 和 I2C 等关键协议的必要性。


总之，本论文介绍了基于 Zynq 的信号发生器和记录仪系统，重点是定制硬件开发。它展示了 Zynq APSoC 在机器状态监测领域的实际应用。该系统可为使用人工缺陷数据评估信号处理算法提供测试平台。未来的工作可能涉及用于振动分析和 ML 加速的附加外设。对定制 IP 的进一步改进可以提高采样率、分辨率和缓冲区容量等参数。对软件和底层库的深入优化也可以提高性能和响应速度。总之，该项目是在 Zynq SoC 上开发嵌入式监控解决方案的宝贵入门。
